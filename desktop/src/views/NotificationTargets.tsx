              ...
              <Show when={isOpen()}>
                <form class="alert-editor notification-target-editor" onSubmit={(event) => { event.preventDefault(); void save(); }}>
                  ...
                  <Show when={draft().formatType === "builtin"} fallback={<label class="wide">Custom JSON template<textarea required rows="7" spellcheck={false} value={draft().template} onInput={(e) => field("template", e.currentTarget.value)} /></label>}>
                    <label class="wide">Built-in format<select value={draft().formatId} onChange={(e) => field("formatId", e.currentTarget.value as NotificationTargetDraft["formatId"])}><option value="generic_json">Generic JSON</option><option value="slack_compatible">Slack-compatible</option><option value="cross_notifier">Cross Notifier</option></select></label>
                  </Show>
