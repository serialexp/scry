...
pub use bitstream::{BitOrder, BitStreamDecoder, BitStreamEncoder, Endianness};
pub use codecs::{register_codec, resolve_codec, Codec};
...
impl std::fmt::Display for BinSchemaError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            BinSchemaError::NotImplemented(msg) => {
                write!(f, "[SCHEMA_MISMATCH] Not implemented: {}", msg)
            }
            BinSchemaError::ContextMissing(field) => write!(
                f,
                "[SCHEMA_MISMATCH] Context missing required field: {}",
                field
            ),
            ...
            BinSchemaError::StackOverflow => {
                write!(f, "[STACK_OVERFLOW] Position stack depth exceeded")
            }
            BinSchemaError::CircularReference => write!(
                f,
                "[CIRCULAR_REFERENCE] Pointer graph contains an infinite loop"
            ),
        }
    }
}
...
pub fn crc32(data: &[u8]) -> u32 {
    ...
    const TABLE: [u32; 256] = [
        ...
        0xDBBBBBD6, 0xACBCCF40, 0x32D86CE3, 0x45DF5C75, 0xDCD60DCF, 0xABD13D59, 0x26D930AC,
