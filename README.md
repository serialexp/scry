...
| **v0.8**  | 🚧     | Size-tiered **compaction** + per-signal **retention** landed. Compaction: standalone `scry-compact` (`--once` / `--watch`) merges the K smallest blocks in a `(signal, date, level)` partition into one at the next level, DataFusion sort-merge, sidecars rebuilt (postings union, logs body bloom, metrics `series_types`); supersede→grace→delete with `superseded_by IS NULL` query-skip making grace=0 safe; `level` promoted into the sidecar (D-036). Retention: standalone `scry-retention` reaps blocks whose newest record is past a per-signal TTL — opt-in (no implicit deletion), **dry-run by default** (`--apply` to delete), whole-block `ts_max` criterion (D-037). Both single-instance; multi-instance lease + the in-`scry-ingestd` background loop still pending. |
