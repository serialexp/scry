...
| **v0.9**  | —      | Make compaction + retention **multi-instance** and self-driving: a per-`(signal, day)` object-store lease so N servers coordinate without stepping on each other (the immutable + content-addressed block design already makes a stale-lease double-op harmless), and an in-`scry-ingestd` background loop that runs both engines as scheduled tasks instead of standalone tools. |
