...
Status: **v0.6 complete — all four signals (metrics, logs, traces, profiles) flow end to end and query back.** A record goes in via the wire protocol (or a foreign protocol terminated by `scry-gateway`: OTLP traces, Pyroscope profiles, Prometheus remote-write), lands in a per-writer WAL, gets uploaded as a parquet block (with a per-signal postings sidecar for metrics/logs; traces/profiles carry none) to Garage, shows up in a SQLite catalog (online or reconciled from the bucket), and is queryable through DataFusion either locally (`scry-query`) or over the `scry-queryd` daemon's binschema-framed wire. v0.1 (storage), v0.2 (metrics ingest+query), v0.3 (query daemon), v0.4 (logs), the unnumbered push-gateway milestone, v0.5 (traces query: `--trace-id` by-id lookup + promoted-column matchers, predicate pushdown), and v0.6 (profiles retrieval query: raw pprof blob out) are all sealed by `scripts/smoke.sh`, which asserts the full ingest → store → **query** round-trip per signal (plus a `--trace-id` lookup for traces). Metrics/logs preselect via postings; traces/profiles push matcher/time/trace-id filters as parquet row predicates and prune on row-group stats. See `docs/decisions.md § D-029…D-034` for the rationale and the README milestone table for scope. Still not implemented: profiles flamegraph aggregation (pprof parse + stack-merge — deferred, Grafana renders pre-aggregated data), PromQL + full-text logs (v0.7), compaction/retention (v0.8), and the multi-bucket / Valkey convergence from `docs/ARCHITECTURE.md`.
